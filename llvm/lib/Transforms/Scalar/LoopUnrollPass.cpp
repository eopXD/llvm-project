...
bool MLGOLoopUnrollAnalysis::flush() {
  std::error_code EC;
  // The append flag is specified here because modules is compiled at the same
  // time during usual compulations and we want the logs to be in the same file.
...
shouldPartialUnroll(const unsigned LoopSize, const unsigned TripCount,
                    const UnrollCostEstimator UCE,
                    const TargetTransformInfo::UnrollingPreferences &UP,
                    ScalarEvolution &SE, LoopInfo &LI, Loop &L) {
  ...
  if (EnableMLGOUnroll)
    MLGOAnalysis->logFeaturesAndDecision(count, L);
...
bool llvm::computeUnrollCount(
  ...
  if (auto UnrollFactor =
          shouldPartialUnroll(LoopSize, TripCount, UCE, UP, SE, *LI, *L)) {
    UP.Count = *UnrollFactor;
...
PreservedAnalyses LoopUnrollPass::run(Function &F,
  ...
  // Creating the MLGOAnalysis object at first sight
...
LoopUnrollPass::~LoopUnrollPass() {
  // At end of compilation, the LoopUnrollPass dtor is triggered. All logs of
  // loop features and partial unroll decisions will be collected at this point
  // and we now dump the logs.
  if (EnableMLGOUnroll)
    MLGOAnalysis->flush();
}
